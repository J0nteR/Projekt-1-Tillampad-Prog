/*
    TO DO!
        önskemål --> lägg till queue för golfbollar / något som kan släppa på endast 1 golfboll i taget NÄR open_gate kallas på
*/
#include <U8glib.h>
#include <Wire.h>
...
int last_press = 0;             // the elapsed ms when the a button was pressed
int ms_since_last_press = 2000; // ms since the last press
...
void setup()
{
  ...
  pinMode(8, INPUT);  // button for gate 3 (red)
  pinMode(9, INPUT);  // button for gate 2 (yellow)
  pinMode(10, INPUT); // button for gate 1 (green)
  Wire.begin();
...
/*
  updates the just_sorted var to the most recent button pressed
  Parameters: none
  Returns: none
...
{
  ms_since_last_press = millis() - last_press;

  if (ms_since_last_press < 2000)
  { // makes sure one button-press doesn't get registred multiple times, by keeping track of the time elapsed since the last press
    return;
  }
  else if (digitalRead(8) == HIGH) //(green)
  {
    just_sorted = 1;
    last_press = millis();
  }
  else if (digitalRead(9) == HIGH) //(yellow)
  {
    just_sorted = 2;
    last_press = millis();
  }
  else if (digitalRead(10) == HIGH) //(red)
  {
    just_sorted = 3;
    last_press = millis();
  }
